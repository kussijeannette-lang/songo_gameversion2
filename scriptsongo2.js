...
// Lance la vérification automatique toutes les 2 secondes
setInterval(chargerEtatDepuisServeur, 2000);

function chargerEtatDepuisServeur() {
    fetch('server.php?action=get_state')
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                // 1. On extrait le plateau (qui est souvent stocké en texte/JSON dans la DB)
                const backendBoard = typeof data.board === 'string' ? JSON.parse(data.board) : data.board;
                
                // 2. IMPORTANT : On met à jour tes variables globales JavaScript locales
                // Remplace "monPlateau", "tourDuJoueur", "scoreJ1", "scoreJ2" par le nom exact de TES variables locales
                monPlateau = backendBoard;
                tourDuJoueur = data.current_player; 
                scoreJ1 = data.score_j1;
                scoreJ2 = data.score_j2;

                // 3. On relance ta fonction existante qui redessine le Songo à l'écran
                // Remplace "redessinerPlateau()" par le nom exact de ta fonction d'affichage
                redessinerPlateau();
            }
        })
